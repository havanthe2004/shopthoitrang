...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '0912345678' }).fill('0123456789');
  await page.getByRole('textbox', { name: 'example@mail.com' }).click();
  await page.getByRole('textbox', { name: 'example@mail.com' }).fill('binhb@gmail.com');
  ...
  await expect(page.getByText(/Số điện thoại không đúng định dạng Việt Nam/)).toBeVisible();
});