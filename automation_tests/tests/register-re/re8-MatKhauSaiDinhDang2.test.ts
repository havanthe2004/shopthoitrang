...
test('test', async ({ page }) => {
  ...
  await page.locator('input[name="password"]').fill('12345678a');
  await page.locator('input[name="confirmPassword"]').click();
  await page.locator('input[name="confirmPassword"]').fill('12345678a');
  await page.getByRole('button', { name: 'Đăng ký ngay' }).click();
  await expect(page.getByText(/Phải chứa ít nhất 1 chữ cái hoa/)).toBeVisible();
});