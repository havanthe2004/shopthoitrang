...
const ProductList = () => {
  ...
  const fetchProducts = async (page = 1) => {
    setLoading(true);
    try {
        // 🔥 TRUYỀN THÊM THAM SỐ isActive VÀO ĐÂY
  ...
  useEffect(() => {
    const timeout = setTimeout(() => fetchProducts(1), 500);
    return () => clearTimeout(timeout);
}, [search, viewMode]); // Tự động load lại khi đổi tab hoặc tìm kiếm
